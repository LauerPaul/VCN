/**
*	@version 1.0 beta
*	@module @/components/banners/banner-full-width
*	@desc <strong>Component Banner full width</strong> - модуль баннера, который отображается во всю ширину родителя
*	@see ~/components/banners/banner-full-width
*
...
const methods = {
	...
	*/
	setData (){
		// Log mounted hook
		this.$log.debug('component \'@/components/banners/banner-full-width\' -> method init');
		...
		if(a){
			...
			// Если не передана ссылка на изображение
				...
				this.$log.warn('component \'@/components/banners/banner-full-width\' -> prop options.image not found or it\'s empty');
			}
		}
		// Если не передана переменная options
		else{
			this.$log.warn('component \'@/components/banners/banner-full-width\' -> prop options not found');
		}
	}
}
...
export default {
	...
	* 
	* @event module:@/components/banners/banner-full-width~COMPONENT <strong>Banner-full-width</strong> mounted
	*/
	mounted: function(){
		// Log mounted hook
		this.$log.info('component \'@/components/banners/banner-full-width\' -> mounted');
		// 
		this.setData();
	},
